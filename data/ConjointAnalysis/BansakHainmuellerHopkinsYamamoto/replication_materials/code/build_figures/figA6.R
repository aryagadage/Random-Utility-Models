...
pdf("../../results/figures/figA6.pdf",width=4,height=9)
ggplot(ssdat,aes(y=ests,x=conds)) + stat_smooth(aes(x=agedummy2,y=agedummy),fullrange=TRUE,method="lm",color="black",size=0.65) +
  ...
  geom_line(color=the.color,linetype=2) + geom_blank(aes(y=fakelow)) + geom_blank(aes(y=fakehigh)) + coord_cartesian(xlim=c(0,5))
dev.off()
