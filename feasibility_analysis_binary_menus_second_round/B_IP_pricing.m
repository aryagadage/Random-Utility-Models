...
%   F* >= F_current - 2*cp,
% so the relative distance gap
%   gap = (sqrt(F_current) - sqrt(F_current - 2*cp)) / sqrt(F_current - 2*cp)
% is <= tol_gap whenever
%   cp <= cp_thr := F_current * (1 - 1/(1+tol_gap)^2) / 2.
% F_current = ||residual||^2 = price'*price is available from the inputs,
% so we ask Gurobi to early-stop via BestBdStop = lb + cp_thr; once the IP
% bound drops below that, the outer FW gap is certified <= tol_gap and
% the pricer returns immediately.
tol_gap   = 0.01;
...
% Optimal value (Gurobi-certified upper bound on the IP optimum)
