function [p_obs, choice_sets, chosen_alts, choice_set_list] = ...
    B_generate_fake_data_all_menus_outside(n, mode, menu_size_spec, rng_seed)
% B_GENERATE_FAKE_DATA_ALL_MENUS_OUTSIDE  Generate synthetic choice data
% for menus of arbitrary sizes with probabilities drawn from a Dirichlet
% distribution (NOT from a random utility model).
%
%   [p_obs, choice_sets, chosen_alts, choice_set_list] = ...
%       B_generate_fake_data_all_menus_outside(n)
%
%   [p_obs, choice_sets, chosen_alts, choice_set_list] = ...
%       B_generate_fake_data_all_menus_outside(n, mode, menu_size_spec, rng_seed)
%
%   For each sampled menu of size s, a probability vector is drawn from
%   Dirichlet(1, 1, ..., 1)  (i.e. uniform on the (s-1)-simplex).
%   This means the data is generally NOT rationalizable by any RUM.
%
%   INPUTS
...
% (no true_utilities in the outside model)
fprintf('Generating fake data for n=%d alternatives, %d menus (Dirichlet probs)...\n', n, n_menus);
...
for t = 1:n_menus
    ...
    % Draw choice probabilities from Dirichlet(1,1,...,1) on the s-simplex
    % Dirichlet(1,...,1) = uniform on the simplex.
    % Method: draw s independent Exp(1) = -log(U) values, then normalise.
    g = -log(rand(s, 1));            % Gamma(1,1) = Exp(1) samples
    probs = g / sum(g);              % Dirichlet draw
