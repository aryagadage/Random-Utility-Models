...
% (Dominick's version: handles arbitrary menu sizes >= 2).
%
%   ip_model = B_IP_pricing_build(n, choice_set_list)
%
% Builds the parts of the Gurobi model that depend only on
% (n, choice_set_list): variable bounds/types and the constraint matrix.
% The four constraint blocks are:
%
%   Block 1: pairwise equality       x_{ij} + x_{ji} = 1         for i < j
%   Block 2: triangle inequality     x_{ij} + x_{jk} + x_{ki} <= 2
%   Block 3: zero constraints        -x_{a,b} + p(D, a) <= 0      (|D| >= 3)
%   Block 4: probability simplex     sum_k p(D, cs(k)) = 1        (|D| >= 3)
%
% Compared to the binary-feasibility version, Blocks 3 and 4 are kept
% active so menus of size >= 3 (the common case in the Dominick's data)
% are wired in. For binary-only menus the size->=3 guards are no-ops and
% the model collapses to (Block 1, Block 2) on n^2 rank vars — same as
% the binary-feasibility build.
%
...
%   choice_set_list - cell array of unique menus (row vectors).
%
...
%% Block 2: triangle inequality   x_{ij} + x_{jk} + x_{ki} <= 2
%   for i < j, k > i, k ~= j (loop over i to bound peak memory)
for i_val = 1:n
    rng_jk = (i_val+1):n;
...
%   For each menu D and each (a, b) with a, b in D, a ~= b.
...
counter = n*n + 1;
for j_cs = 1:nm
    cs = choice_set_list{j_cs}(:);   % column so cs(idx) preserves shape
    s  = numel(cs);
    if s < 3
        continue;   % binary menus contribute no zero constraints (no prob vars)
    end
    ...
    prob_cols = counter + k_loc - 1;           % +1 columns (per-element prob)
